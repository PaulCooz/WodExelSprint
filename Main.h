...
#pragma region Windows Form Designer generated code
	...
	private: System::Void ClearXlsxButton_Click(System::Object^ sender, System::EventArgs^ e) {
		...
		for each (Worksheet ^ worksheet in worksheets)
		{
			auto name = worksheet->Name;
			auto checkForEmptyCol = 0;
			auto numbersVisited = 0;
			while (numbersVisited < 3) {
				auto str = sheet->GetStr(worksheet, 2, ++checkForEmptyCol);
				auto isNum = System::Text::RegularExpressions::Regex::IsMatch(str, "-?\\d+[\\.,]?\\d*");
				if (isNum)
					numbersVisited++;
			}
